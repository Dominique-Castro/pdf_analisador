[11:34, 06/06/2025] Dominique Castro: import streamlit as st
...
def gerar_relatorio(encontrados, nao_encontrados, data_acidente=None, numero_processo=None):
    ...
    doc.add_page_break()
    doc.add_paragraph("_______________")
...
if uploaded_file is not None:
    try:
        ...
        with tab2:
            with st.container(border=True):
                if nao_encontrados:
                    st.markdown(f"*{len(nao_encontrados)} documentos não e…
[11:34, 06/06/2025] Dominique Castro: old code:
[11:34, 06/06/2025] Dominique Castro: import streamlit as st
from pdf2image import convert_from_bytes
import pytesseract
from docx import Document
